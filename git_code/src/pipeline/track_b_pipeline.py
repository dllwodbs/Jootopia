...
def run_track_b_pipeline(
    ...
    # L0: 유니버스
    uni_path = cal_data_dir / "universe_k200_membership_monthly"
    ...
    cv_short_path = cal_data_dir / "cv_folds_short"
    ...
    scores_path = (
        cal_data_dir / f"rebalance_scores_from_ranking_interval_{rebalance_interval}"
    )
    ...
    if artifact_exists(scores_path) and not force_rebuild:
        ...
        ohlcv_path = cal_data_dir / "ohlcv_daily"
        ohlcv_daily = None
    ...
    # 결과 저장
    suffix = f"_{strategy}"
    save_artifact(bt_pos, cal_data_dir / f"bt_positions{suffix}", force=True)
    save_artifact(bt_ret, cal_data_dir / f"bt_returns{suffix}", force=True)
    save_artifact(bt_eq, cal_data_dir / f"bt_equity_curve{suffix}", force=True)
    save_artifact(bt_met, cal_data_dir / f"bt_metrics{suffix}", force=True)
    ...
    if (
        ...
        save_artifact(
            bt_returns_diagnostics,
            cal_data_dir / f"bt_returns_diagnostics{suffix}",
            force=True,
        )
    ...
    if (
        ...
        save_artifact(
            selection_diagnostics,
            cal_data_dir / f"selection_diagnostics{suffix}",
            force=True,
        )
    ...
    if runtime_profile is not None:
        ...
        try:
            ...
            rp_path = cal_data_dir / f"runtime_profile{suffix}.json"
            rp_path.write_text(
    ...
    if (
        ...
        save_artifact(
            bt_regime_metrics, cal_data_dir / f"bt_regime_metrics{suffix}", force=True
        )
        artifacts_path["bt_regime_metrics"] = str(
            cal_data_dir / f"bt_regime_metrics{suffix}"
        )

    artifacts_path["bt_positions"] = str(cal_data_dir / f"bt_positions{suffix}")
    artifacts_path["bt_returns"] = str(cal_data_dir / f"bt_returns{suffix}")
    artifacts_path["bt_equity_curve"] = str(cal_data_dir / f"bt_equity_curve{suffix}")
    artifacts_path["bt_metrics"] = str(cal_data_dir / f"bt_metrics{suffix}")
